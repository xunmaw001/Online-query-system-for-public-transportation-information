<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="周边分类" prop="zhoubianfenlei">
          <el-select v-model="ruleForm.zhoubianfenlei" placeholder="请选择周边分类">
            <el-option
                v-for="(item,index) in zhoubianfenleiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/zhoubianfenlei/zhoubianfenlei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.zhoubianfenleiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../ssm3957q/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhoubianxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhoubianxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
