...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','zhoubianfenlei');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','zhoubianfenlei');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('zhoubianfenlei', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusszhoubianfenlei/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('zhoubianfenlei', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('zhoubianfenlei', this.detail);
				this.$utils.msg('审核成功');
